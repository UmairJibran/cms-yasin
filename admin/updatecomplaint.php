...
$sql = "SELECT `userId` FROM `tblcomplaints` WHERE `complaintNumber`= '$complaintnumber'";
if ($result = mysqli_query($con,$sql)) {
  while ($row = $result -> fetch_row()) {
    mysqli_query($con,"INSERT INTO `notif` (`id`, `userid`, `prid`) VALUES (NULL, '$row[0]', '$complaintnumber')");
  }
  $result -> free_result();
}
